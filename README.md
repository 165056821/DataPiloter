...
- 🚀 [React](https://react.dev) 用于前端。
  - 💃 使用 TypeScript、hooks、[Vite](https://vitejs.dev) 和其他现代前端技术栈组件。
...
你可以（也应该）从 secrets 中将这些作为环境变量传递。
...
或者更好的是，如果你有 [`pipx`](https://pipx.pypa.io/)，你可以运行：
...
- `smtp_host`：（默认：""）发送邮件的 SMTP 服务器主机，你可以稍后在 .env 中设置。
- `smtp_user`：（默认：""）发送邮件的 SMTP 服务器用户，你可以稍后在 .env 中设置。
- `smtp_password`：（默认：""）发送邮件的 SMTP 服务器密码，你可以稍后在 .env 中设置。
- `emails_from_email`：（默认：`"info@example.com"`）发送邮件的邮箱账户，你可以稍后在 .env 中设置。
